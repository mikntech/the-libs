...
openAIQueue.process(async (job) => {
  ...
  try {
    ...
    const user = await getUserModel().findById(
      (await getAITaskModel().findById(taskID))?.userID,
    );
    ...
    if (prompt) {
      ...
      await Promise.all(promises).then(async (updatedPropmtResult) => {
        ...
        if (res === -1) throw new Error('Acoount error');
          ...
          const similarToInvalidBad = stringSimilarity(
            completion.choices[0].message?.content + '',
            'Invalid Idea.',
          );
          const similarToInvalidBad2 = stringSimilarity(
            completion.choices[0].message?.content + '',
            'Invalid input.',
          );

          if (
            similarToInvalidGood > 0.4 ||
            similarToInvalidBad > 0.4 ||
            similarToInvalidBad2 > 0.4
          ) {
            axiosInstance
